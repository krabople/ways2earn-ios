...
const candidates = bundles.data?.filter(item => item.attributes?.identifier === 'com.krabople.ways2earn') ?? [];
const bundle = candidates.find(item => item.attributes?.platform === 'IOS') ?? candidates.find(item => item.attributes?.platform === 'UNIVERSAL');
