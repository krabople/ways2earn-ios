    ...
    return (
      <Screen title="Alerts">
        <MessageState
          ...
          body="Sign in to see replies, mentions, messages and saved searches."
        />
  ...
  return (
    <Screen
      ...
      action={
        feed?.unread ? (
          <Pressable
            onPress={() => void action({ action: "readNotifications" })}
          >
    ...
    >
          ...
          return (
            ...
            >
              ...
              <View style={styles.copy}>
                ...
                <Text style={styles.meta}>{age(item.created_at)}</Text>
              </View>
      ...
      {!alerts.length ? (
        <MessageState
          ...
          body="Replies, mentions, moderation updates and alert matches will appear here."
        />
      ) : null}
    </Screen>
  );
...
const styles = StyleSheet.create({
  readAll: { color: colours.green, fontWeight: "800", fontSize: 12 },
  list: { gap: spacing.sm },
  alert: {
    flexDirection: "row",
    alignItems: "center",
  ...
  unread: { borderColor: "#A8DFC9", backgroundColor: colours.mintPale },
  dot: { width: 9, height: 9, borderRadius: 5, backgroundColor: colours.mint },
  dotRead: { backgroundColor: "#C5CED3" },
  copy: { flex: 1, gap: 4 },
  title: {
    color: colours.ink,
    fontSize: 14,
  ...
  chevron: { color: colours.slate, fontSize: 27 },
  primary: {
    minHeight: 48,
    alignItems: "center",
