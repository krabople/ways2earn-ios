...
export const API_ORIGIN = process.env.EXPO_PUBLIC_API_ORIGIN ?? 'https://www.ways2earn.com';
...
export async function request<T>(path = '', body?: Record<string, unknown>): Promise<T> {
  ...
  if (!response.ok) throw new Error(result.error || 'Ways2Earn could not complete that request.');
  return result;
}
...
export async function signOut() {
  try {
    await request('/session', { action: 'logout' });
  } finally {
...
export const mutate = <T = { ok: true }>(body: Record<string, unknown>) => request<T>('', body);

export async function uploadImage(uri: string) {
  const bearer = await token();
  const data = new FormData();
  data.append('image', { uri, name: 'ways2earn-image.jpg', type: 'image/jpeg' } as unknown as Blob);
